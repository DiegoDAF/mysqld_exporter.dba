# mysqld_exporter.dba — SSH tunnel fork

This is a **fork of [prometheus/mysqld_exporter](https://github.com/prometheus/mysqld_exporter)** adding native SSH tunnel support. All upstream behavior is preserved when the new YAML config is not used.

Sibling project: [pgscv.dba](https://github.com/DiegoDAF/pgscv.dba) (same pattern for PostgreSQL).

## What this fork adds

- **SSH tunnel support** for monitoring MySQL instances reachable only via bastion/jump hosts (common in RDS/Aurora setups).
- **YAML config** (`--config.yaml`) as an alternative to `.my.cnf`, with the same layout as `pgscv.dba` for operational consistency.
- **Zero regression** when `--config.yaml` is not provided — behaves identically to upstream.
...
services:
  "my-mysql-prod":
    service_type: "mysql"
    dsn_file: "/run/secrets/my-mysql.dsn"
    ssh_tunnel:
      ...
      # known_hosts_file: ""          # optional, skip verification if empty
      # keepalive_seconds: 30         # default
```

The DSN file contains a single-line MySQL DSN:

```
exporter:password@tcp(mysql-db.internal.example.com:3306)/
```

Run:

```bash
mysqld_exporter --config.yaml=/etc/mysqld_exporter/config.yaml
```

When the YAML config is used, `.my.cnf` is ignored. When the YAML flag is empty, upstream behavior is preserved exactly.

## How it works

1. At startup, the exporter reads the YAML config and establishes a persistent SSH tunnel to the bastion, forwarding a local port to the remote MySQL.
2. The DSN host:port is rewritten to `127.0.0.1:<local_port>`.
3. `sql.Open("mysql", dsn)` connects through the tunnel transparently.
4. Keepalives maintain the tunnel; if it drops, it's automatically re-established on the next scrape.
...
```bash
docker build -f Dockerfile.dba -t mysqld-exporter-dba:latest .
```

Run with Docker Compose (sidecar-free, tunnel is internal):

```yaml
services:
  mysqld-exporter:
    image: ghcr.io/diegodaf/mysqld_exporter.dba:latest
    command: ["--config.yaml=/etc/mysqld_exporter/config.yaml"]
    volumes:
      - ./config.yaml:/etc/mysqld_exporter/config.yaml:ro
      - ./secrets/my-mysql.dsn:/run/secrets/my-mysql.dsn:ro
...
# Upstream README (prometheus/mysqld_exporter)
