...
var (
	...
	configYaml = kingpin.Flag(
		"config.yaml",
		"Path to optional YAML config for SSH tunnel support (opt-in, bypasses --config.my-cnf).",
	).Default("").String()
...
func newHandler(scrapers []collector.Scraper, logger *slog.Logger, yamlDSN string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// YAML mode: use pre-resolved (and possibly tunnel-rewritten) DSN directly,
		// bypassing .my.cnf parsing. Preserves backward compat when --config.yaml is not set.
		if yamlDSN != "" {
			dsn = yamlDSN
		} else {
			cfg := c.GetConfig()
...
func main() {
	...
	// If --config.yaml is set, load it and establish SSH tunnel (if configured).
	// This sets yamlDSN which overrides the .my.cnf DSN path at request time.
	// When --config.yaml is empty, behavior is 100% upstream-compatible.
	var yamlDSN string
	var tunnelMgr *tunnel.Manager
	if *configYaml != "" {
		yamlCfg, err := config.LoadYAMLConfig(*configYaml)
		if err != nil {
			logger.Error("Error loading yaml config", "file", *configYaml, "err", err)
			os.Exit(1)
		}
		if len(yamlCfg.Services) != 1 {
			logger.Error("yaml config must have exactly one service", "found", len(yamlCfg.Services))
			os.Exit(1)
		}
		tunnelMgr = tunnel.NewManager(context.Background())
		var serviceID string
		var svc *config.ServiceConfig
		for id, s := range yamlCfg.Services {
			serviceID = id
			svc = s
		}
		yamlDSN, err = svc.PrepareDSN(context.Background(), tunnelMgr, serviceID)
		if err != nil {
			logger.Error("Error preparing DSN from yaml config", "service", serviceID, "err", err)
			os.Exit(1)
		}
		logger.Info("YAML config loaded, DSN prepared", "service", serviceID, "ssh_tunnel", svc.SSHTunnel != nil)
	}

	var err error
	// Only load .my.cnf when YAML mode is NOT active.
	if yamlDSN == "" {
	...
	handlerFunc := newHandler(enabledScrapers, logger, yamlDSN)
